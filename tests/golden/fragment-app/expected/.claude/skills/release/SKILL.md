---
name: release
description: fragment-app 프로젝트의 릴리스를 수행한다. 사용자가 릴리스해줘, 버전 올려, bump, 태그 따줘, release, 릴리스 준비됐는지 봐줘, 다음 버전 뭐가 될까 등 버전 결정·태그·GitHub Release·릴리스 노트와 관련된 요청을 하면 반드시 이 스킬을 사용한다.
---
<!-- generated by superrelease v0.1.0 — do not edit; re-run init to regenerate (직접 수정하지 말고 재init 하세요) -->

# release — fragment-app 릴리스 오케스트레이터
...
노트 소스로 `changelog.d/*.md` 조각을 category별로 취합하라 — 파일명 `{id}.{category}.md`의 category가 `breaking`이면 Breaking Changes, `feature`면 하이라이트·변경 사항, `fix`·`misc`(및 미인식)이면 변경 사항에 넣는다. 취합한 조각 파일은 릴리스 커밋에서 `git rm`으로 삭제하고 6단계 프리뷰에 명시하라(bump 결정에는 쓰지 않는다 — bump는 커밋·PR 소스 그대로).
