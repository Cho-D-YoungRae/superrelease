---
name: release
description: calver-app 프로젝트의 릴리스를 수행한다. 사용자가 릴리스해줘, 버전 올려, bump, 태그 따줘, release, 릴리스 준비됐는지 봐줘, 다음 버전 뭐가 될까 등 버전 결정·태그·GitHub Release·릴리스 노트와 관련된 요청을 하면 반드시 이 스킬을 사용한다.
---
<!-- generated by superrelease v0.1.0 — do not edit; re-run init to regenerate (직접 수정하지 말고 재init 하세요) -->

# release — calver-app 릴리스 오케스트레이터
...
- calver 체계에는 bump 수준 개념이 없다 — 다음 버전은 날짜·카운터가 결정한다: `python3 .superrelease/scripts/next-version.py` (config의 scheme·pattern을 자동 사용). 변경 내용은 2단계 수집분으로 노트에만 반영하고 버전 결정에는 쓰지 않는다.
