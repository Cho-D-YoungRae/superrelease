---
name: hotfix
description: hotfix-library 프로젝트의 이미 릴리스된 버전에 핫픽스 패치 릴리스를 수행한다. 사용자가 핫픽스, hotfix, 긴급 패치, 구버전 패치, 유지보수 라인 릴리스, 1.2.x에 패치 등 과거 릴리스 수정과 관련된 요청을 하면 반드시 이 스킬을 사용한다.
---
<!-- generated by superrelease v0.1.0 — do not edit; re-run init to regenerate (직접 수정하지 말고 재init 하세요) -->

# hotfix — hotfix-library 유지보수 라인 패치 릴리스
...
확인 후: 버전 파일 + 노트 파일을 커밋하고 `git push origin release/<라인>`.
