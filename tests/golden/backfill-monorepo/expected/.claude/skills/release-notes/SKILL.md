---
name: release-notes
description: backfill-monorepo 모노레포의 패키지별 릴리스 노트 초안을 작성한다. 사용자가 릴리스 노트 써줘, 특정 패키지 릴리스 정리해줘, 체인지로그 정리, changelog, release notes 등 변경 요약·노트 작성 관련 요청을 하면 반드시 이 스킬을 사용한다. release 스킬이 5단계에서 재사용한다.
---
