---
name: release
description: backfill-monorepo 모노레포의 패키지 릴리스를 수행한다. 사용자가 릴리스해줘, 특정 패키지 릴리스, 버전 올려, bump, 태그 따줘, release, 어떤 패키지 바뀌었어, 릴리스 준비됐는지 봐줘 등 버전 결정·태그·GitHub Release·릴리스 노트와 관련된 요청을 하면 반드시 이 스킬을 사용한다.
---
<!-- generated by superrelease v0.1.0 — do not edit; re-run init to regenerate (직접 수정하지 말고 재init 하세요) -->

# release — backfill-monorepo 모노레포 릴리스 오케스트레이터 (independent)
...
- 수집: `git log <anchor>..HEAD --pretty=format:"%h %s" -- <scope.path>`
