...
머지 후 재개: 1단계 preflight 6(중단 상태 감지)이 이 대기 상태를 잡는다. PR 머지 여부를 확인(`gh pr view <PR번호> --json state`)한 뒤 `git checkout main && git pull`로 머지 커밋을 받아 7단계(태그)부터 이어가라 — 태그는 머지 후 HEAD에 만든다(squash 머지로 sha가 바뀐다). PR이 아직 열려 있으면 대기 중임을 보고하고 멈춰라.
