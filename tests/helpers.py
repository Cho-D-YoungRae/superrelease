...
_MARKER_VERSION_RE = re.compile(r"(generated by superrelease v)\d+\.\d+\.\d+")


def normalize_marker_version(text):
    """The generated marker embeds a plugin version (config.superrelease.pluginVersion,
    with a plugin.json fallback). Normalize it so golden and self-render byte
    comparisons stay stable even if that stamp ever drifts — a defensive backstop on
    top of the config-sourced marker."""
    return _MARKER_VERSION_RE.sub(r"\g<1>X.Y.Z", text)
