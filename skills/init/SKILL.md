...
- 브랜칭: trunk / gitflow(단일 스킬 레포 · release-pr 전용 — develop cut → 기본 브랜치 태그 → back-merge 정식 사이클, production hotfix 포함) 지원 — 모노레포×gitflow·direct-push gitflow는 지원하지 않는다
...
- 노트 목적지: changelog/release-file/github-release/fragment 지원 — fragment는 sink 동반 필수(render가 검증) / tag-message(annotated 태그 메시지에 노트 전문)는 지원하지 않는다 — 태그 메시지에는 한 줄 요약만 남는다
- hotfix 스킬: semver 단일 스킬 레포 지원(independent 모노레포는 지원하지 않음) / CHANGELOG backfill: 단일 scope·independent 모노레포 지원(모노레포는 scope별 `<scope>@<version>` 순회, 전 scope tagless면 render 거부)
