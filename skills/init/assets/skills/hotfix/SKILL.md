...
- {{#if scope.postRelease.bump == "next-snapshot"}}back-merge 후 `{{repo.developBranch}}`에서 다음 개발 버전으로 복귀한다: `python3 .superrelease/scripts/next-version.py --bump patch --qualifier {{scope.preRelease.qualifier}}` → `version.py set` → 5단계와 같은 경로로 프리뷰·확인 후 커밋·push.{{else}}post-release bump 없음 — 파일 버전은 릴리스 버전 그대로 둔다.{{/if}}{{#each scope.notes.destinations}}{{#if this == "changelog"}}
