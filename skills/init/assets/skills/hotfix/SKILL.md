...
{{#if repo.branching == "gitflow"}}- 현 production은 `{{repo.defaultBranch}}`의 최신 릴리스다(gitflow: 릴리스 후 `{{repo.defaultBranch}}`는 릴리스 버전을 유지한다). {{#if derived.anyTagEnabled}}최신 릴리스 태그를 확인한다: `git -c versionsort.suffix=- tag --list '<glob>' --sort=-v:refname | head -n 1` (`<glob>`은 config `scopes[].tag.format`의 `{version}`을 `*`로 치환).{{else}}태그를 쓰지 않으므로 현 버전은 파일에서 읽는다: `python3 .superrelease/scripts/version.py get{{#if repo.monorepoStrategy == "independent"}} --scope <name>{{/if}}`.{{/if}}
...
{{/if}}- 수정이 이미 `{{repo.developBranch}}`/`{{repo.defaultBranch}}`에 커밋돼 있는지(체리픽 대상 sha), hotfix 브랜치에서 직접 고칠지 확인한다.{{else}}- 고칠 릴리스 버전을 확인한다(예: 1.2.3). 그 버전의 태그가 존재해야 한다 — 태그명은 config `scopes[].tag.format`의 {version} 대입.
...
{{#if repo.branching == "gitflow"}}- 범위 anchor는 {{#if derived.anyTagEnabled}}`{{repo.defaultBranch}}`의 최신 릴리스 태그: `git -c versionsort.suffix=- tag --list '<glob>' --sort=-v:refname | head -n 1` (`<glob>`은 config `scopes[].tag.format`의 `{version}`을 `*`로 치환){{else}}`origin/{{repo.defaultBranch}}`다(태그를 쓰지 않는다){{/if}}{{else}}- 범위 anchor는 이 라인의 마지막 태그: 라인 체크아웃 상태에서 `git describe --tags --abbrev=0 --match '<glob>'` (`<glob>`은 config `scopes[].tag.format`의 `{version}`을 `*`로 치환 — 이 라인에서 도달 가능한, 포맷에 맞는 태그만){{/if}}
- 버전 반영: `python3 .superrelease/scripts/version.py set <패치 버전>{{#if repo.monorepoStrategy == "independent"}} --scope <name>{{/if}}`
- 노트: `.claude/skills/release-notes/SKILL.md` 절차로 초안을 쓰고, config `scopes[].notes.destinations` 목적지 반영은 release 스킬 5단계와 동일하게 하라.
{{#if bundle.enabled}}- **bundle 라운드 노트**: hotfix도 라운드다 — `{{bundle.notesPath}}`의 `*.md` 파일명(확장자 제거)으로 `python3 .superrelease/scripts/next-version.py --scheme calver --pattern {{bundle.scheme.pattern}} --today <오늘 YYYY-MM-DD> --current-among <파일명들…>` 라운드 번호를 계산하고(파일이 없으면 `--current ""`), `.superrelease/templates/notes-bundle.md` 골격으로 `{{bundle.notesPath}}<라운드>.md`를 작성해 hotfix 커밋에 포함하라(수정된 scope만 표에 "이번 라운드"로 표시).
{{/if}}
## 5. dry-run 프리뷰 → 커밋
...
{{#if derived.anyTagEnabled}}## 6. 태그{{#if github.release}} + GitHub Release{{/if}}

release 스킬 {{#if repo.monorepoStrategy == "independent"}}8단계와 동일하다(`tag.enabled`인 scope만, scope별 `tag.format`){{else}}7단계와 동일하다{{/if}}: `git ls-remote --tags origin <태그>`로 충돌 재확인(결과가 있으면 즉시 중단) → 태그 생성·push{{#if github.release}} → Release 생성{{/if}}.{{#if github.release}}
...
- {{#if scope.postRelease.bump == "next-snapshot"}}back-merge 후 `{{repo.developBranch}}`에서 다음 개발 버전으로 복귀한다(기본 patch 증가, `{{repo.developBranch}}`가 이미 앞선 minor면 조정 확인): `python3 .superrelease/scripts/next-version.py --bump patch --qualifier {{scope.preRelease.qualifier}}{{#if repo.monorepoStrategy == "independent"}} --scope <name>{{/if}}` → `version.py set{{#if repo.monorepoStrategy == "independent"}} --scope <name>{{/if}}` → 5단계와 같은 경로로 프리뷰·확인 후 커밋·push.{{#if repo.monorepoStrategy == "independent"}} mutable scope마다 반복한다.{{/if}}{{else}}post-release bump 없음 — 파일 버전은 릴리스 버전 그대로 둔다.{{/if}}{{#each scope.notes.destinations}}{{#if this == "changelog"}}
