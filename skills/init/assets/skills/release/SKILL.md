...
{{/if}}{{/if}}{{#if scope.tag.enabled}}{{#if repo.branching == "gitflow"}}6. 중단 상태 감지 (gitflow): ① `gh pr list --state merged --search "head:release/" --json headRefName,mergedAt` 결과의 `release/<버전>` head 중 그 버전의 태그가 아직 없으면 이전 릴리스가 태그 전에 중단된 것 — 6단계의 "머지 후 재개"대로 7단계(태그)부터 이어가라. ② 기본 브랜치에 병합된 최신 릴리스 태그(`git -c versionsort.suffix=- tag --list '<glob>' --merged origin/{{repo.defaultBranch}} --sort=-v:refname | head -n 1`)가 현재 브랜치에서 도달 불가하면(`git merge-base --is-ancestor <태그> HEAD` 실패) 직전 릴리스의 back-merge가 누락된 것 — 8단계의 back-merge부터 복구하라 (유지보수 라인 태그는 `--merged`로 제외된다).{{else}}6. 중단 상태 감지: 마지막 릴리스 태그가 존재하고 파일 버전이 그보다 높은데 **파일 버전 그대로의 태그가 없으면** 이전 릴리스가 중단된 것이다{{#if scope.preRelease.style == "mutable"}} (단, 파일 버전에 `-{{scope.preRelease.qualifier}}` 수식어가 붙어 있으면 정상 개발 상태 — 중단 아님){{/if}} — 이어서 진행(resume)/되돌리기(rollback) 중 사용자 선택을 받아라.{{/if}}
