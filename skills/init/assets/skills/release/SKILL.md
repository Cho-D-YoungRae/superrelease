...
3. **여기서 중단한다** — 태그·Release는 PR 머지 후다. "PR이 머지되면 다시 릴리스를 요청하세요"라고 안내하라.{{#if repo.branching == "gitflow"}} 릴리스 PR은 **머지 커밋**(`gh pr merge --merge`, --no-ff)으로 머지하라 — squash로 머지하면 태그 조상에서 `{{repo.developBranch}}` 이력이 끊겨 다음 릴리스의 범위·bump가 과거 릴리스분을 재수록한다. 머지 후에는 태그(7단계)와 `{{repo.developBranch}}` back-merge(8단계)가 남아 있음을 함께 알려라.{{/if}}
...
{{#if repo.branching == "gitflow"}}태그 push 후 **back-merge**로 `{{repo.developBranch}}` 브랜치를 동기화한다: `git checkout {{repo.developBranch}} && git pull` → `git merge {{repo.defaultBranch}}` (충돌이 나면 해결안을 사용자와 확인하며 진행 — 버전 파일 충돌은 `{{repo.defaultBranch}}` 쪽 릴리스 버전을 취하라, 직후 복귀 단계가 덮는다) → 프리뷰·확인 후 `git push origin {{repo.developBranch}}`. push가 거부되면(`{{repo.developBranch}}` 보호) back-merge PR을 만들어 머지하라.
