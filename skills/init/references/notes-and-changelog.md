...
| fragment | `changelog.d/` 조각 → 취합 | M3 |
| tag-message | annotated 태그 메시지 | M3 |
...
- `fragment + changelog + github-release`(M3): 여러 기여자가 각자 조각으로 노트를 쓰고, 이를 모아 누적 CHANGELOG와 GitHub Release 두 곳에 동시에 반영하는 팀.
...
release-file이 M1에 포함된 이유는 렌더링 비용이 낮기 때문이다 — 이미 갖고 있는 노트를 파일로 한 번 더 쓰는 것뿐이라 추가 구현 부담이 거의 없다. 반면 fragment는 조각 파일 규약과 취합 로직을, tag-message는 별도의 릴리스 플로우 연동을 새로 설계해야 해서 M3로 미뤄졌다.
