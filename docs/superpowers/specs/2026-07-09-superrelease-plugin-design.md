...
```json
{
  ...
  "author": { "name": "Youngrae Cho", "email": "yrcho@aimskr.com" },
...
{
  "name": "superrelease",
  "owner": { "name": "Youngrae Cho", "email": "yrcho@aimskr.com" },
  "plugins": [
    {
      "name": "superrelease",
      "source": "./",
...
**Claude가 gh CLI로 직접 확인** (인증 의존, 저볼륨): protected branch + required checks, 기존 Releases, `.github/release.yml` 존재, `gh auth status` 스코프. gh 미설치/미인증이면 해당 질문에서 GitHub 연동 기능을 안내와 함께 제한 모드로 제안.
...
1. **preflight** — clean working tree / **현재 브랜치 == `repo.defaultBranch`** (hotfix는 예외적으로 release/* 라인) / 원격과 동기화(fetch 후 비교) / `version.py verify` 통과 / (GitHub Release 사용 시) gh auth 스코프 — **gh 미가용이면 "태그까지만 진행" 제한 모드 제안** / 태그 충돌 없음 / **중단 상태 감지**: 파일 버전 > 마지막 태그인데 미태깅이면 이전 릴리스 중단으로 판단 → resume/rollback 제안
...
7. **태그 + GitHub Release** — annotated 태그(짧은 요약을 태그 메시지에), `gh release create`(`--generate-notes` 뼈대 + LLM 가공 노트). 태그 push가 CI 배포 트리거인 레포면 프리뷰에 "이 태그는 배포를 트리거합니다" 명시
...
- JSON 계열은 네이티브 명령 우선(`npm pkg get/set`), 불가 시 파이썬 json 모듈(키 순서·들여쓰기·마지막 개행 보존). sed로 JSON을 조작하지 않는다
- **npm lockfile 동기화**: package.json 버전 수정 후 lockfile 존재 + npm 가용 시 `npm install --package-lock-only` 실행
...
2. 요구사항 — Claude Code, Python 3.9+(생성 스크립트 실행), gh CLI(GitHub Release 사용 시)
...
10. FAQ — 플러그인 없는 팀원도 되나(생성 스킬·스크립트는 자립적) / Windows에서 python3(`py -3`) / 제거 방법(`.superrelease/` + `.claude/skills/release*` 삭제) / 배포(publish)는 왜 안 하나(태그 트리거 CI 권장 — 가이드 문단) / 잘못 나간 버전 회수
11. 로드맵 — M1/M2/M3
...
- **unittest (Python stdlib만, `tests/`)**:
  ...
  - version.py: location 타입 3종 get/set/verify, npm lockfile 동기화(모킹), 포맷 보존
...
| package.json 포맷 훼손 | npm 네이티브 우선 + 보존형 json writer + dry-run에서 diff 표시 |
