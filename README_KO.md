...
| `.claude/skills/release-train/SKILL.md` | 루트 release-train 스킬 (조건부: 이중 체계 `train`) |
...
| `.superrelease/templates/notes-train.md` | 릴리스 트레인 노트 골격 (조건부: 이중 체계 train) |
...
| `scopes[].notes.destinations` | `changelog` \| `release-file` \| `github-release` \| `fragment` \| `tag-message` | `fragment`는 sink 필요; `tag-message`는 annotated/signed 태그 필요 |
...
- **제거하려면?** `.superrelease/`와
  `.claude/skills/{release*,hotfix,backfill,release-train}`를 삭제하세요
