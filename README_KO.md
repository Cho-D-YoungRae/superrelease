...
- 정직한 한계: 모노레포 × gitflow와 루트 CalVer 묶음 버전(release-train
  스타일)은 **지원하지 않습니다** — 모노레포는 기본 브랜치에서
  릴리스합니다(direct-push 또는 release-pr). 아래 *지원 계획 없음* 참고.
...
| gitflow | `develop` 통합 브랜치에서 릴리스하는 팀 | 단일 레포(비모노레포)·release-pr 전용 (develop에서 cut → main에 머지 → 태그 → back-merge) |

gitflow 지원은 release-pr 경로의 단일 레포(비모노레포)로 한정됩니다;
모노레포 × gitflow와 direct-push gitflow는 지원하지 않습니다.
...
지원 계획 없음(범위 밖): sequential 버저닝, 모노레포 × gitflow, direct-push
gitflow, 릴리스 트레인, `tag-message` 노트, `pom.xml` project `<version>` 직접
